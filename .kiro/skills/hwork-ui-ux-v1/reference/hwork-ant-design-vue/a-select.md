...
| listHeight | 设置弹窗滚动高度 | number | 212 | - |
...
| searchTrigger | 控制搜索事件触发场景，注：设置 typing 后只要用户输入就会触发，包括中文输入法合成文字的情况。而默认情况下则将忽略输入法合成文字时的输入场景 | auto typing | auto | - |
| searchValue | 控制搜索文本 | string | - | - |
| showArrow | 是否显示下拉小箭头 | boolean | true | - |
| showSearch | 配置是否可搜索 | boolean | 单选为 false,多选为 true | - |
| showSelectAll | 是否展示全选选项 | string \| boolean | - | - |
| size | 选择框大小，可选 large small mini | string | default | - |
...
| prefix | 前缀 | string \| slot \| VNode | - | h-work 新增 |
...
四种大小的选择框，当 size 分别为 `large`、`small`、`mini` 时，输入框高度为 `40px`、`28px`、`24px`，默认高度为 `32px`。

### 自动分词

试下复制内容到输入框里。只在 tags 和 multiple 模式下可用。
...
使用 showSelectAll 属性在列表中简单得增加一个选择全部的选项，或者使用 Select.SELECT_ALL_VALUE 进行完全的自定义。

注：该功能仅在 mode="multiple" 时生效。

## FAQ

### 点击 dropdownRender 里的内容浮层关闭怎么办？

自定义内容点击时会关闭浮层，如果不喜欢关闭，可以通过取消点击事件的默认行为进行阻止。看下 dropdownRender 例子里的说明。

### 为什么 placeholder 不显示？

placeholder 只有在 value = undefined 才会显示，对于其它的 null、0、'' 等等对于 JS 语言都是有意义的值。

### 为何受控模式下，value 可以超出范围？

在受控模式下，开发者可能自行存储相关数据。如果组件将数据约束回范围内，会导致展示数据与实际存储数据不一致的情况。
