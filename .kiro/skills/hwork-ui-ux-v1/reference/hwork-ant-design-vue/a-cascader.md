...
| optionLabel | 通过 optionLabel 插槽，自定义节点展示内容 | v-slot:optionLabel="data: Option" | - | - |
| options | 可选项数据源 | Option[] | - | - |
| placeholder | 输入框占位文本 | string | '请选择' | - |
| placement | 浮层预设位置 | bottomLeft \| bottomRight \| topLeft \| topRight | bottomLeft | 3.0 |
| showCheckedStrategy | 定义选中项回填的方式。Cascader.SHOW_FULL_PATH: 只显示选中的子节点（包含该节点的完整路径）。 Cascader.SHOW_CHILD: 只显示选中的子节点。Cascader.SHOW_PARENT: 只显示父节点（当父节点下所有子节点都选中时）。 | Cascader.SHOW_FULL_PATH \| Cascader.SHOW_PARENT \| Cascader.SHOW_CHILD | Cascader.SHOW_PARENT | 3.3.0 |
...
| prefix | 自定义的选择框前缀内容 | string \| slot \| VNode | - | - |
...
> 注意：默认情况下，选中父级的所有子元素后，`v-model:value` 以及 `change` 事件的返回值将仅包含父级元素本身。 若希望直接返回子级的数据，请设置 `show-checked-strategy` 值为 `Cascader.SHOW_CHILD` 或 `Cascader.SHOW_FULL_PATH`。
